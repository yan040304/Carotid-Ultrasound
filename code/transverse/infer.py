...
def postprocess_mask(mask_tensor, original_size):
    ...
    mask = (mask > 0.5).astype(np.uint8) * 255  # 二值化并转换为0-255范围
...
if __name__ == "__main__":
    # === 路径配置 ===
    model_path = "code/transverse/carotid_unet.pth"
    input_dir = "data/transverse/image"
    label_dir = "data/transverse/label"  # 标签目录
    output_dir = "results/deep_learning/transverse"
    viz_dir = "results/deep_learning/transverse_visualizations"
    results_csv_path = "results/diagnosis/transverse/dl_evaluation_results.csv"
